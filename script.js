document.addEventListener('DOMContentLoaded', () => {
    ...
    if (hamburgerMenu && navLinksContainer) {
        hamburgerMenu.addEventListener('document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetAndInitializeScrollObserver(container) {
        ...
        const observerOptions = {
            root: null,
            rootMargin: '0px',
            // HUOM: Muutettu threshold takaisin pienemmäksi (0.2).
            // Tämä saa animaatiot käynnistymään herkemmin ja luo paremman porrastusefektin,
            // kun useampi elementti laukeaa "melkein" samanaikaisesti.
            threshold: 0.2 
        };

        currentObserver = new IntersectionObserver((entries, observer) => {
            
            // Käydään läpi vain ne elementit, jotka ovat tulossa näkyviin
            const intersectingEntries = entries.filter(e => e.isIntersecting);

            intersectingEntries.forEach((entry, index) => {
                if (!entry.target.classList.contains('visible')) {
                    
                    // --- AUTOMAATTINEN PORRASTUS (UUSI TAPA) ---
                    // Annetaan kullekin samassa "satsissa" ilmestyvälle elementille
                    // pieni kasvava viive (100ms, 200ms, 300ms...)
                    ...
                    }, delay); // Käytetään uutta porrastettua viivettä
                }
            });
        }, observerOptions);
